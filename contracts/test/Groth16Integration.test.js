...
const hasCircuitArtifacts = fs.existsSync(path.join(buildDir, "proof.json")) &&
                            fs.existsSync(path.join(buildDir, "public.json")) &&
                            fs.existsSync(path.join(buildDir, "verification_key.json"));

describe("MedicalDataManager + Groth16Verifier (integration)", function () {
  before(function () {
    if (!hasCircuitArtifacts) {
      this.skip();
    }
  });
